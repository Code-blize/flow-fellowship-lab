...
A custom strategy class was created to intercept and record metrics after each round:

```python
class LoggingStrategy(fl.server.strategy.FedAvg):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.metrics_log = []  # Critical: initialize the list
...
- **Standard deviation** to measure client consistency
...
**Design benefits:**
- Single function runs any optimizer configuration
- Deterministic seeds ensure reproducibility
- Strategy handles all metric collection
- Clean separation of experiment logic from visualization
